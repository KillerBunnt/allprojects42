...
int algo1(t_table *hold)
{
	...
	while (1)
	{
		sortstart(hold, startmaxsorted->value, endmaxsorted->value, 10);
		sortend(hold);
	}
	return (0);
}

int sortstart(t_table *hold, int startend, int endend, int count)
{
	while (hold->stacka->prev->value != endend && --count)
	{
